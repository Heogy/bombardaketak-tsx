    ...
    return new Promise((resolve, reject) => {
        ...
        fs.readFile(sqlDumpFile, 'utf8', (err, sql) => {
            ...
            db.exec(sql, (err) => {
                ...
                db.close((closeErr) => {
                    if (closeErr) return reject(`Error closing database: ${closeErr.message}`);
                    resolve();
                });
            });
        });
        ...
        db.all("SELECT name FROM sqlite_master WHERE type='table'", (err, tables) => {
            ...
            tables.forEach(({ name }) => {
                db.all(`SELECT * FROM ${name}`, (err, rows) => {
                    if (err) {
                        db.close();
                        return reject(`Error fetching data from table ${name}: ${err.message}`);
                    }
                    ...
                    if (tablesProcessed === tables.length) {
                        fs.writeFile(outputFile, JSON.stringify(dbData, null, 4), (err) => {
                            db.close();
                            if (err) return reject(`Error writing JSON file: ${err.message}`);
